...
void sendToServer(String mode, unsigned long duration, float temp = NAN, float humi = NAN) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (!isnan(temp) && !isnan(humi)) {
      ...
      jsonPayload += "\"humidity\": " + String(humi, 2) + ",";
    }
    jsonPayload += "\"message\": \"Data update\"";
    jsonPayload += "}";
...
void loop() {
  ...
  if (millis() - lastSensorSendTime >= sensorSendInterval) {
    ...
    if (!isnan(temp) && !isnan(humi)) {
      ...
      sendToServer(currentMode, millis() - modeStartTime, temp, humi);
    } else {
